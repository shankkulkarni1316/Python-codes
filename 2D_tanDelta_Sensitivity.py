...
from LHS_2parameters import LHSampaling100ValuesFor2Parameters
from part import *
...
#--------------------------------------------------------------------------------------
# finding parameters for sensitivity analysis
YoungsModulusVariation = LHSampaling100ValuesFor2Parameters(0)
YoungsModulusVariation_Inclusion = []
YoungsModulusVariation_Matrix = []

for i in range (0,100):
    YoungsModulusVariation_Inclusion.append(0.5 + (1.0*YoungsModulusVariation[i][0]))
    YoungsModulusVariation_Matrix.append(0.001 + ((1000-0.001)*YoungsModulusVariation[i][1]))
#---------------------------------------------------------------------------------------
...
for max_incl in allInclusions:
    ...
    for ik in range (25,50):        
        ...
        mdb.models['Model-1'].materials['VE'].Elastic(table=((
            0.9436270853*YoungsModulusVariation_Matrix[ik], 0.49214), ))        
        ...
        mdb.models['Model-1'].materials['Elastic'].Elastic(table=
            ((64890*YoungsModulusVariation_Inclusion[ik], 0.249), ))            
        ...
        for w in range (1,W):
            ...
            mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
                ...
                multiprocessingMode=DEFAULT, name='Job-%d-%d-%d' %(max_incl,ik,w) , nodalOutputPrecision=SINGLE, 
                numCpus=1, queue=None, scratch='', type=ANALYSIS, userSubroutine='', 
                waitHours=0, waitMinutes=0)                
            mdb.jobs['Job-%d-%d-%d' %(max_incl,ik,w) ].submit(consistencyChecking=OFF)
            #mdb.jobs['Job-%d-%d-%d' %(max_incl,ik,w)].writeInput()
